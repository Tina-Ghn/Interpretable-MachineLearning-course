...
def test_get_custom_gradient():
    ...
    assert torch.allclose(gradient.mean(), torch.tensor(0.0), atol=1e-4)
    assert torch.allclose(gradient.sum(), torch.tensor(-0.0662), atol=1e-3)
    ...
    assert torch.allclose(gradient.sum(), torch.tensor(56.4033), atol=1e-3)
...
def test_get_custom_integrated_gradients():
    ...
    assert torch.allclose(ig.mean(), torch.tensor(0.0), atol=1e-4)
    assert torch.allclose(ig.sum(), torch.tensor(0.9176), atol=1e-2)
