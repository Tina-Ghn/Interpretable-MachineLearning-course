...
def create_counterfactual(model, x, desired_y, lambda_reg=1.0, num_steps=1000):
    ...
    Starting from a copy of the instance x, uses the Adam optimizer (with default params) to find a counterfactual of
    class desired_y. Optimizes for a maximum of num_steps steps. Stops the optimization if the predicted class has
    changed to the desired one.

    Parameters:
        model (MLPClassifier, torch.nn.Module):
            The classifier to generate a counterfactual for. Is a torch.nn.Module, so you can call its forward method
            using `model()`. Its output are logits (unnormalized class probabilities), which have to be normalized by
            applying a softmax operation.
        ...
        lambda_reg (float): The regularization strenght used by the loss function.
