...
class Dataset:
    ...
    def get_lower_and_upper_bound_for_feature(self, feature_index: int):
        hps = self.get_configspace().get_hyperparameters_dict()

        labels = self.get_input_labels()

        return hps[labels[feature_index]].lower, hps[labels[feature_index]].upper
