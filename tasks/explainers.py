...
def get_integrated_gradients(model, image):
    """
    Uses captum's IntegratedGradients method to generate an attribution map w.r.t. the model's prediction as the target. Uses zeros (black image) as the baseline, that are normalized using 'normalize_image'.
